...
export default function LoginPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setLoading(true); setError(""); const result = await login(email, password, remember); setLoading(false); if (!result.ok) return setError(result.error); router.push("/dashboard"); };
